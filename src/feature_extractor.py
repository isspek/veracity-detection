...
class FeatureExtractor:
    def __init__(self):
        ...
        with open(config.get_badwords(), 'r') as f:
            for line in f:
                self.badwords.append(line.strip().lower())

        with open(config.get_negative_smileys(), 'r') as f:
            for line in f:
                self.negative_smileys.append(line.strip())

        with open(config.get_positive_smileys(), 'r') as f:
            for line in f:
...
'__CARDINAL__']
    ...
    def extract_aux_feats(self, item, args,embeddings):
        aux_feats = []
        text = self.preprocess(item.post.raw_text)
        if 'post_role' in args:
            aux_feats.append(self.post_role(item.post))
        ...
        if 'similarity' in args:
            if len(text) == 0:
                ...
                if item.post.has_source_depth == 0:
                    aux_feats.append(1)
                else:
                    aux_feats.append(self.similarity(item.post.id, item.post.source_id, embeddings))
        if 'num_url' in args:
            aux_feats.append(self.filter_url(text)[1])
        if 'num_mention' in args:
            aux_feats.append(self.filter_mention(text)[1])
        if 'num_hashtag' in args:
            aux_feats.append(self.filter_hashtag(text)[1])

        tokens = self.other_tokenizer(self.preprocess(text))
        if 'badwords' in args:
...
if __name__ == '__main__':
    ...
    sdqc_dev = rumour_dataset.sdqc_dataset.dev
    args = {}
    args['aux_features'] = ['post_role', 'sentiment_analyzer']
